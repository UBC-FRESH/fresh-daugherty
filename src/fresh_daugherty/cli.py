...
def grid_value_flow(
    ...
    denominators: str = typer.Option(
        "volume,revenue", "--denominators", help="Comma-separated flow-row denominators."
    ),
    horizon: int = typer.Option(15, "--horizon", min=1),
    workers: int = typer.Option(
        1, "--workers", min=1, help="Parallel processes (grid is embarrassingly parallel)."
    ),
    out: Path = typer.Option(Path("results") / "experiments" / "grid_value_flow.csv", "--out"),
) -> None:
    """Run the E3 value-flow grid (landbase x rate x policy x flow denominator).

    Each cell is a full sequential-replanning simulation under the policy's
    bounded-deviation flow constraint denominated in volume or undiscounted
    net revenue, with the objective-gap diagnostic and dual (volume + revenue)
    trajectory records. Writes the per-cell summary to ``--out``, trajectories
    to ``<out-stem>_trajectories.csv``, gap records to ``<out-stem>_gaps.csv``.
    """
    from fresh_daugherty.experiments import run_value_flow_grid
    from fresh_daugherty.instance.reconstruct import calibrate
    from fresh_daugherty.instance.thesis import HARVEST_FLOW_POLICIES
    ...
    pol_by_code = {p.code: p for p in HARVEST_FLOW_POLICIES}
    pols = tuple(pol_by_code[c] for c in policies.split(","))
    denoms = tuple(d.strip() for d in denominators.split(","))
    summary, trajectories, gaps = run_value_flow_grid(
        landbases=lbs,
        discount_rates=rates,
        policies=pols,
        denominators=denoms,
    ...
    typer.echo(f"wrote {out} ({len(summary)} cells)")
    for d in denoms:
        sub = summary[summary["flow_denominator"] == d]
