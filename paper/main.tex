...
All software, extracted case-study data, experiment configurations, the full per-cell simulation records, and the validation records supporting this paper are openly available in the \texttt{fresh-daugherty} repository at \url{https://github.com/UBC-FRESH/fresh-daugherty} (this draft: commit \texttt{bddd1ea} on branch \texttt{feature/p8-rigorous-cjfr}). The complete benchmark record---the grid summary and the per-cell projected and realized harvest trajectories for all 432 cells, together with the exact regeneration command and pinned dependency versions---is tracked under \texttt{results/experiments/}; a versioned release archiving these artifacts with a DOI (Zenodo) will be minted and cited in the final version. The Umpqua planning documents are public-domain US Forest Service works accessible via HathiTrust (records 002547999 and 002439528).
